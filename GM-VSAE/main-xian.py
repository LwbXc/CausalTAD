...
class Trainer:
    ...
    def __init__(self, config, save_path, cuda_devices=[1], load_model=None) -> None:
        ...
        self.model = Model(self.label_num, self.params.hidden_size, self.params.layer_num)
        ...
        self.optimizer1 = Adam([p for p in self.model.parameters() if (p is not self.model.model.pi_prior) and (p is not self.model.model.mu_prior) and (p is not self.model.model.log_var_prior)], lr=self.params.lr, weight_decay=self.params.weight_decay)
        self.optimizer2 = Adam([self.model.model.pi_prior, self.model.model.mu_prior, self.model.model.log_var_prior], lr=self.params.lr, weight_decay=self.params.weight_decay)
    ...
    def train(self, epochs: int, train_dataloader: TrajectoryLoader, valid_dataloader: dict):
        ...
        for epoch in range(epochs):
            ...
            for i, data in enumerate(train_dataloader.src_data_batchs):
                ...
                loss = self.model.forward(src, trg, src_lengths, trg_lengths, train=True)
                avg_loss += loss.item()
            ...
            for k,dataloader in valid_dataloader.items():
                for i, data in enumerate(dataloader.src_data_batchs):
                    ...
                    nll_loss = self.model.forward(src, trg, src_lengths, trg_lengths, train=False)
    ...
    def test(self, normal_dataloader, abnormal_dataloaders):
        ...
        print('GM-VSAE', '|'.join(map(str, pr_list)))
